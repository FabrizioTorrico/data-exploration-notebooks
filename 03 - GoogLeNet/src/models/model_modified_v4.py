...
class ParallelStateBlock(nn.Module):
    def __init__(self, in_channels, out_1x1, red_3x3, out_3x3, red_5x5, out_5x5, out_pool_proj, 
        ...
        # LÓGICA DE PROYECCIÓN DE ESTADO (CORRECCIÓN)
        dim_concat_3x3 = out_3x3 + out_pool_proj # Dimensión actual
        # Si nos dicen cuánto mide el anterior, usamos eso. Si no, asumimos que es igual al actual (para el primer bloque)
    ...
    def forward(self, x, prev_state_3x3=None, prev_state_5x5=None):
        ...
        if prev_state_3x3 is not None:
            # Aquí la self.recurrence_3x3 ahora maneja el cambio de dimensión
...
class SpatialResidualBlock(nn.Module):
    """
    Bloque estándar para fases posteriores (sin memoria recurrente).
    """
    ...
    def forward(self, x):
        ...
        res = self.shortcut(x)
        # Importante: Si las dimensiones de concat y res no coinciden, fallará.
        # En esta config me aseguro que 'total_out' sea igual a la suma de ramas.
...
class GoogLeNetModifiedV4(nn.Module):
    def __init__(self, in_channels=3, num_classes=1000, aux_logits: bool = False):
        ...
        # Mantenemos lógica, ajustes leves.
        ...
        # --- REDUCCIÓN 3 (DIETA APLICADA) ---
        # Antes sumaba 256 canales. Ahora sumamos 160.
        # In: 704. Out: 704(pool) + 160(conv) = 864
        self.reduction3 = ReductionBlock(704, 160)


        # --- FASE 2: SPATIAL (Ajuste fino para ~9.9M) ---
...
if __name__ == "__main__":
    # Batch size 2, 3 canales RGB, imagen 224x224
